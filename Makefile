...
VERSION_PACKAGE=github.com/iWuxc/miniblog/pkg/version
## 定义 VERSION 语义化版本号
...
ca: # 生成 CA 文件.
	...
	@openssl req -new -nodes -key $(OUTPUT_DIR)/cert/ca.key -sha256 -days 3650 -out $(OUTPUT_DIR)/cert/ca.csr \
		-subj "/C=CN/ST=Beijing/L=Beijing/O=miniblog/OU=it/CN=127.0.0.1/emailAddress=wuxc.eng@gmail.com"
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/server.key -out $(OUTPUT_DIR)/cert/server.csr \
		-subj "/C=CN/ST=Beijing/L=Beijing/O=serverdevops/OU=serverit/CN=localhost/emailAddress=wuxc.eng@gmailcom" \
